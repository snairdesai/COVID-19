...
# ***** This has been commented out for now, because of the poor quality of the data. 
# However, the dataframe is updated with the newest results each time the chunk is run. 
# If we want to re-implement testing in some way, we can uncomment and run this chunk. *****
```{r testing}
...
# Now, I add our controls of cross-country travel.
```{r travel}
Final_Data_Country <- Final_Data_Country[order(Final_Data_Country$COUNTRY, Final_Data_Country$Date),]
...
# Now, I add additional country-specific characteristics I have gathered from the World Bank. All data is from 2018, unless noted.
```{r world bank}
Final_Data_Country <- Final_Data_Country[order(Final_Data_Country$COUNTRY, Final_Data_Country$Date),]
...
# Lastly, I perform our final edits and save the dataframe.
```{r editing}
...
#  Note that I change the column types so reader can properly parse the data. This dataset also includes lagged variables for the mortality rate, which are used in later analysis.
...
```{r spatial}
# Now, we want to integrate our spatial datasets from Landscan. 
...
## Creating an interactive choropleth map for overall confirmed cases and deaths (Bloomberg Plot 2). I use the temporary data, which includes the complete set of nations. This code has been commented out (the Cloud doesn't like the shape file format), but the output is available in the COVID-19_Final folder.
...
New_Mortality <- ggplot(data = New_Global_Mortality, aes(x = Date, y = New_Mortality)) +
      ...
      labs(title = "New Global Mortality Rate - COVID19",
           subtitle = "January 22nd, 2020 - April 28th, 2020",
           x = "Date", y = "New Mortality Rate Per Capita") + theme_bw()
...
``` {r Rashad setup - April 26th 2020}
...
### HTML output of the regressions
...
### HTML output of the regressions:elderly
```{r,warning=FALSE, results='hide',message=FALSE}
...
#### heterogeneity: Proportion Urban
```{r}
###baseline projection regressions, remove 'infinite' values. two-way FE, HAC robust SEs, clustered on country
...
### HTML output of the regressions:urban
```{r,warning=FALSE, results='hide',message=FALSE}
...
### HTML output of the regressions: latitude
```{r,warning=FALSE, results='hide',message=FALSE}
...
### HTML output of the regressions:longitude
```{r,warning=FALSE, results='hide',message=FALSE}
...
###HTML output of the regressions:density
```{r,warning=FALSE, results='hide',message=FALSE}
...
###HTML output of the regressions:log(Arrivals)
```{r,warning=FALSE, results='hide',message=FALSE}
...
###HTML output of the regressions:log(Departures)
```{r,warning=FALSE, results='hide',message=FALSE}
...
#### heterogeneity: Vulnerable Employees
```{r}
###baseline projection regressions, remove 'infinite' values. two-way FE, HAC robust SEs, clustered on country
...
###HTML output of the regressions:vulnerable employees
```{r,warning=FALSE, results='hide',message=FALSE}
...
###HTML output of the regressions:GNI
```{r,warning=FALSE, results='hide',message=FALSE}
...
#### heterogeneity:health expenditure
```{r}
###baseline projection regressions, remove 'infinite' values. two-way FE, HAC robust SEs, clustered on country
...
###HTML output of the regressions:Health exp
```{r,warning=FALSE, results='hide',message=FALSE}
...
###HTML output of the regressions:Pollution
```{r,warning=FALSE, results='hide',message=FALSE}
...
### Weining Cross-Sectional and Panel Analysis
...
## Why does bb only have Vietnam as a recorded country?
...
### Plot K-P curves for confirmed cases.
```{r}
...
### Cox proportionaal hazard regression
```{r}
